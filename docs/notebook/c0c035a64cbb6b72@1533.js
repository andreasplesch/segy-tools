// https://observablehq.com/d/c0c035a64cbb6b72@1533
...
Only works with Chrome or Firefox >= 65`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`## 1. Provide file`
)});
...
html`<input type=file accept='.sgy,.segy'>`
  ...
  main.variable(observer()).define(["md","currentFile","segybuffer_byteLength"], function(md,currentFile,segybuffer_byteLength){return(
md`
File ** ${currentFile.name} ** has ** ${segybuffer_byteLength} ** bytes.
`
)});
  main.variable(observer("progress")).define("progress", ["totalRead","currentFile","md"], function(totalRead,currentFile,md)
{
  const done = totalRead.total / currentFile.size;
  return md`** ${( done * 100 + 1000).toFixed(0).slice(-3)}% ** out of ${currentFile.size} bytes processed: <progress style='min-width: 300px; height: 0.2em; vertical-align: middle;' value=${done}></progress>`;
}
);
  main.variable(observer()).define(["md","binaryHeader","formatMap","totalTraceNo"], function(md,binaryHeader,formatMap,totalTraceNo){return(
md`
...
number({value: Math.floor(totalTraceNo/2), title: "trace to analyse", description: "trace 1 to "+totalTraceNo, step:1, min:1, max:totalTraceNo, submit:0})
)});
...
function (halfWindow = 1000/2, maxRecords = 200, maxSamples = 600, trace = 1){
  ...
  const trEnd = Math.min(totalTraceNo, trStart + 2*halfWindow);
  ...
  main.variable(observer("amplitudePlot")).define("amplitudePlot", ["traceRecords","centerTrace","width","plotContrast","vegalite"], function(traceRecords,centerTrace,width,plotContrast,vegalite)
{
  const data = traceRecords(250, 200, 600, centerTrace);
  ...
  //const markWidth = (plotWidth+0)/(traces);
  const clipFactor = 1 - plotContrast;
...
//      _height:markHeight,
  ...
  main.variable(observer("viewof plotContrast")).define("viewof plotContrast", ["number"], function(number){return(
number({value: 0.2, title: "contrast in plot", description: "0 to 0.9", step:0.1, min:0.0, max:0.9, submit:1})
)});
  main.variable(observer("plotContrast")).define("plotContrast", ["Generators", "viewof plotContrast"], (G, _) => G.input(_));
  main.variable(observer("viewof centerTrace")).define("viewof centerTrace", ["number","totalTraceNo"], function(number,totalTraceNo){return(
number({value: totalTraceNo>>1, title: "center trace to plot", description: "trace 1 to "+totalTraceNo, step:100, min:1, max:totalTraceNo, submit:1})
)});
  main.variable(observer("centerTrace")).define("centerTrace", ["Generators", "viewof centerTrace"], (G, _) => G.input(_));
  main.variable(observer()).define(["md"], function(md){return(
...
md`## Supporting Cells`
  ...
  main.define("initial segybuffer", ["totalRead"], function(totalRead)
{
  const buffer = totalRead.readBuffer.buffer;
  if (buffer.byteLength == totalRead.total) return buffer;
  //const buffer = currentFile.arrayBuffer();
  //return readBuffer;
//   const buffer = totalRead.readBuffer.buffer;
//   if (buffer.byteLength == totalRead.total) yield buffer;
  ...
  main.variable(observer("mutable segybuffer")).define("mutable segybuffer", ["Mutable", "initial segybuffer"], (M, _) => new M(_));
  main.variable(observer("segybuffer")).define("segybuffer", ["mutable segybuffer"], _ => _.generator);
  main.define("initial segybuffer_byteLength", function(){return(
"Loading ..."
)});
  main.variable(observer("mutable segybuffer_byteLength")).define("mutable segybuffer_byteLength", ["Mutable", "initial segybuffer_byteLength"], (M, _) => new M(_));
  main.variable(observer("segybuffer_byteLength")).define("segybuffer_byteLength", ["mutable segybuffer_byteLength"], _ => _.generator);
  main.variable(observer("isLoading")).define("isLoading", ["mutable segybuffer_byteLength","segybuffer","segybuffer_byteLength"], function($0,segybuffer,segybuffer_byteLength)
{
  $0.value = segybuffer.byteLength;
  return segybuffer_byteLength
}
);
  main.variable(observer("totalRead")).define("totalRead", ["currentFile"], async function*(currentFile)
{
  if (await currentFile.stream() == undefined) {
    const buffer = await currentFile.arrayBuffer();
    yield {total: buffer.byteLength, readBuffer: new Uint8Array(buffer)};
    return
  }
  const stream = await currentFile.stream();
  ...
  yield {total: total, readBuffer: readBuffer};
  
  while (true) {
      const {done, value} = await reader.read();
      reads++;
      if (done) break;
      readBuffer.set( value, total );
      total += value.length;
      if (reads%1000 == 0) yield {total: total, readBuffer: readBuffer};
  }
  
  yield {total: total, readBuffer: readBuffer};
}
);
  main.define("initial readBuffer", ["totalRead"], function(totalRead)
{
  const buffer = totalRead.readBuffer.buffer;
  if (buffer.byteLength == totalRead.total) return buffer;
}
);
  main.variable(observer("mutable readBuffer")).define("mutable readBuffer", ["Mutable", "initial readBuffer"], (M, _) => new M(_));
  main.variable(observer("readBuffer")).define("readBuffer", ["mutable readBuffer"], _ => _.generator);
  main.variable(observer("currentFile")).define("currentFile", ["defaultFile","Generators","segyfile","mutable segybuffer_byteLength"], async function*(defaultFile,Generators,segyfile,$0)
{
  yield defaultFile;
  for await (const file of Generators.input(segyfile)) {
    $0.value = "(please wait while loading ...)";
    yield file;
  }
}
);
  main.variable(observer()).define(["currentFile"], function(currentFile){return(
URL.createObjectURL(currentFile)
)});
  main.variable(observer("defaultFile")).define("defaultFile", ["FileAttachment"], function(FileAttachment)
{
  const file = FileAttachment("stk00819t.first5.sgy");
  file.size = 64800; // manually add size
  return file;
}
);
...
md`### for trace samples`
  ...
  main.variable(observer("traceSamples")).define("traceSamples", ["formatMap","binaryHeader","segybuffer","getTraceOffsetValuesInt1632","ibm2js"], function(formatMap,binaryHeader,segybuffer,getTraceOffsetValuesInt1632,ibm2js)
{
  return function(trace) {
    const stride = formatMap[binaryHeader.format].size;
    const view = new DataView( segybuffer, 3600 + (trace - 1) * (240 + binaryHeader.hns * stride) + 240, binaryHeader.hns * stride);
    ...
    for (let i=0; i<ns; i++) {
      ...
      else if (binaryHeader.format == 8) {
        sample = view.getInt8(i4);
      }
      samples.push(sample);
    }
    return {
